...
# Import and register environments from envs folder
#import objectrl.utils.environment.envs.simple_env

evogym_mappings = {
    "evogym-walker": "Walker-v0", #EASY - WORKS
    "evogym-climber": "Climber-v0", # MEDIUM - WORKS
    "evogym-upstepper": "UpStepper-v0", # MEDIUM - TBD, BUT SHOULD WORK
    "evogym-thrower": "Thrower-v0", # MEDIUM - TBD, BUT SHOULD WORK
    "evogym-lifter": "Lifter-v0", # HARD - TBD, BUT SHOULD WORK

    #"evo-walker": "objectrl.utils.environment.envs.simple_env:SimpleWalkingEnv-v0", # FIXME - surely there's just no way its gotta be this explicit
}
    ...
    def _make_single_env():
        if env_name in gymnasium_mujoco_mappings.values():
            if env_name in list(gym.envs.registry.keys()):
                env = gym.make(env_name, render_mode="rgb_array", max_episode_steps=1000 if eval_env else 400)
            else:
        ...
        elif env_name in evogym_mappings.values():
            import evogym.envs # import and register json environments
            ...
            # Create an EvoWorld object with all objects defined in the provided .json file (Test.json has 'robot' and 'ground')
            #world = EvoWorld.from_json(os.path.join('objectrl/utils/environment/world_data', 'Test.json')) # Test.json has 'robot' object included
            #self.world.add_from_array('robot', body, 1, 1, connections=connections) # use this if chosen world_data file does not include a 'robot'
            #object = WorldObject.from_json(os.path.join('objectrl/utils/environment/evogym/world_data', 'object1.json')) # This can be used if the .json file contains only ONE (1) object
            
            #robot = world.objects.get('robot') # get robot from EvoWorld object
            robot = WorldObject.from_json(os.path.join('objectrl/utils/environment/robot_data', f'{env_name}.json')) # get WorldObject from .json file
            #robot = WorldObject.from_json(os.path.join('objectrl/utils/environment/robot_data', 'four_legs.json')) # get WorldObject from .json file

            body = robot.get_structure()
            connections = robot.get_connections()

            env = gym.make(     # make evaluation env - rgb_array used to save video of evaluation run
                    env_name, 
                    body=body, 
                    connections=connections, 
                    max_episode_steps=1000, # number of steps taken in an evaluation environment
                    #render_mode='human',
                    render_mode='rgb_array',
                    #render_options={'verbose': True}, # this print rps and avg rps per run - only makes sense with render_mode='human'
                ) if eval_env else gym.make(    # make training env - no need for rgb_array to save video
                    env_name, 
                    body=body, 
                    connections=connections,
                    #render_mode='human',
                    max_episode_steps=500, # number of steps taken in a training environment
                )
